#!/usr/bin/env python3
"""
SESIÓN 29: Test de Robustez Geométrica

OBJETIVO: Comparar degradación de accuracy ante perturbaciones geométricas
          entre el modelo warped y el modelo original.
...
MÉTRICA DE ÉXITO: Warped debe degradar MENOS en al menos 7/10 perturbaciones.
...
def get_perturbation_transforms():
    """Define todas las perturbaciones geométricas a probar."""
    ...
    for angle in [5, 10, 15]:
        perturbations[f'rotacion_{angle}'] = transforms.Compose([
            GrayscaleToRGB(),
            transforms.Resize((224, 224)),
            transforms.RandomRotation(degrees=(angle, angle)),  # Rotación fija
            transforms.ToTensor(),
            base_normalize
        ])
    ...
    for scale in [0.9, 1.1]:
        perturbations[f'escala_{scale}'] = transforms.Compose([
            GrayscaleToRGB(),
            transforms.Resize((224, 224)),
    ...
    # Traslaciones
    for trans in [0.05, 0.10]:
        perturbations[f'traslacion_{int(trans*100)}pct'] = transforms.Compose([
            GrayscaleToRGB(),
            transforms.Resize((224, 224)),
    ...
    # Flip horizontal
    perturbations['flip_horizontal'] = transforms.Compose([
        GrayscaleToRGB(),
        transforms.Resize((224, 224)),
    ...
    # Combinado leve (rotación 5°, escala 0.95, traslación 3%)
    perturbations['combinado_leve'] = transforms.Compose([
        GrayscaleToRGB(),
        transforms.Resize((224, 224)),
        transforms.RandomRotation(degrees=5),
        transforms.RandomAffine(degrees=0, scale=(0.95, 0.95), translate=(0.03, 0.03)),
        transforms.ToTensor(),
        base_normalize
    ])

    # Combinado fuerte (rotación 10°, escala 0.9, traslación 5%)
    perturbations['combinado_fuerte'] = transforms.Compose([
        GrayscaleToRGB(),
        transforms.Resize((224, 224)),
        transforms.RandomRotation(degrees=10),
        transforms.RandomAffine(degrees=0, scale=(0.9, 0.9), translate=(0.05, 0.05)),
        transforms.ToTensor(),
        base_normalize
    ])
...
def run_robustness_test():
    """Ejecuta el test de robustez geométrica completo."""
    print("=" * 80)
    print("SESIÓN 29: TEST DE ROBUSTEZ GEOMÉTRICA")
    print("=" * 80)
    print("Comparando degradación ante perturbaciones: ORIGINAL vs WARPED")
    print()
    ...
    print("\n4. Evaluando con perturbaciones geométricas...")
    print("-" * 80)
    ...
    if wins_warped >= 7:
        conclusion = "HIPÓTESIS CONFIRMADA: Warped es MÁS ROBUSTO (gana en ≥70% de perturbaciones)"
        hypothesis_confirmed = True
    elif wins_original >= 7:
        conclusion = "HIPÓTESIS REFUTADA: Original es MÁS ROBUSTO (gana en ≥70% de perturbaciones)"
    ...
    results_path = OUTPUT_DIR / 'geometric_robustness_results.json'
