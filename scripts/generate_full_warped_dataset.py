#!/usr/bin/env python3
"""
Sesión 25: Generación de Dataset Warped Completo (~15K imágenes)

Este script:
1. Predice landmarks para todas las imágenes del dataset COVID-19_Radiography_Dataset
2. Aplica warping con margin_scale=1.05 (óptimo encontrado)
3. Genera dataset warped listo para entrenamiento

Dataset original:
- COVID: 3,616 imágenes
- Normal: 10,192 imágenes
- Viral Pneumonia: 1,345 imágenes
- Total: 15,153 imágenes (excluimos Lung_Opacity para mantener 3 clases)
...
# Configuración
MARGIN_SCALE = 1.05  # Óptimo encontrado en experimento
IMAGE_SIZE = 224
BATCH_LOG_INTERVAL = 100

# Paths
FULL_DATASET_DIR = PROJECT_ROOT / "data" / "dataset" / "COVID-19_Radiography_Dataset"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "full_warped_dataset"
...
CLASS_MAPPING = {
    'COVID': 'COVID',
    'Normal': 'Normal',
    'Viral Pneumonia': 'Viral_Pneumonia'  # Normalizar nombre
}
...
def process_and_save_image(predictor, image_path, output_path,
    ...
    try:
        ...
        # Nota: predict() retorna en escala de imagen original, necesitamos normalizado
        ...
        warped = piecewise_affine_warp(
            ...
            triangles=triangles,
            use_full_coverage=False  # Solo área pulmonar
        )
...
def main():
    print("="*70)
    print("SESIÓN 25: GENERACIÓN DE DATASET WARPED COMPLETO")
    print("="*70)
    ...
    # 2. Crear splits
    print("\n2. Creando splits train/val/test...")
