...
(reducción de error de 9.08 a 3.71 píxeles, mejora del 59\%), clasificación
de COVID-19 (98.73\% accuracy con modelo warped), generalización interna
(mejora de 2.4× en ratio de transferencia, corregido en Sesión 39), y robustez a perturbaciones
(30× más resistente a compresión JPEG). El experimento de control (Sesión 39)
reveló que la robustez proviene \textasciitilde75\% de reducción de información y
\textasciitilde25\% de normalización geométrica. Se discuten las limitaciones
...
\midrule
\textbf{Gap} & 3.17\% & 7.70\% & \textbf{2.4×} \\
\bottomrule
\end{tabular}
\end{table}

\begin{resultadoimportante}[title={Mejora en generalización interna (Corregido Sesión 39)}]
El modelo entrenado en imágenes warped muestra:
\begin{itemize}
    \item \textbf{Gap de generalización}: 3.17\% (warped) vs 7.70\% (original)
    \item \textbf{Ratio}: 2.4× mejor generalización (corregido de 11× en Sesión 39)
    \item \textbf{Metodología}: Cross-evaluation con 3 clases idénticas en ambos datasets
    \item \textbf{Nota}: El claim anterior de 11× era inválido por comparar 4 vs 3 clases
\end{itemize}
\end{resultadoimportante}
...
Generalización interna & 7.70\% gap & 3.17\% gap & 2.4× mejor (Sesión 39) \\
...
\begin{itemize}
    \item Gap de generalización: 3.17\% (warped) vs 7.70\% (original)
    \item Mejora: 2.4× en transferencia bidireccional (corregido Sesión 39)
...
Generalización interna & Warped + ResNet-18 & 2.4× mejor transferencia \\
...
\midrule
\textbf{Total adicional} & +30 ms, +4M params & 2.4× generalización, 30× robustez \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Elementos visuales}:
\begin{itemize}
    \item H1 (Generalización): ✓ Confirmada - 2.4× mejor (corregido)
...
\begin{enumerate}
    ...
    generalización interna}: Con una mejora de 2.4× en el ratio de transferencia
    entre dominios (corregido en Sesión 39), el warping produce representaciones más transferibles.
