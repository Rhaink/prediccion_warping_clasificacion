...
\begin{table}[htbp]
    \centering
    \caption{Comparación de rendimiento entre ResNet-18 y EfficientNet-B0 en imágenes normalizadas geométricamente.}
    \label{tab:comparacion_resnet_efficientnet}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Arquitectura} & \textbf{Accuracy} & \textbf{F1 Macro} & \textbf{Parámetros} & \textbf{Tiempo/Época} \\
        \midrule
        ResNet-18 & 99.10\% & 98.45\% & 11.2M & $\sim$40s \\
        EfficientNet-B0 & 97.76\% & 96.80\% & 4.0M & $\sim$100s \\
        \bottomrule
    \end{tabular}
\end{table}

Basándose en estos resultados, se seleccionó \textbf{ResNet-18} como la arquitectura principal del clasificador debido a su mejor rendimiento en el conjunto de datos normalizado (99.10\% vs 97.76\%) y su menor tiempo de entrenamiento. Aunque EfficientNet-B0 ofrece un modelo más compacto, la diferencia de 1.34 puntos porcentuales en accuracy favoreció la selección de ResNet-18.
