...
Con los parámetros seleccionados (\texttt{margin\_scale}=1.05), el método produce una tasa de llenado (\textit{fill rate}) de aproximadamente 47\%, correspondiente a la región pulmonar sin el fondo circundante.

La Figura \ref{fig:warping_comparison} muestra la comparación visual entre la imagen original y la imagen normalizada geométricamente mediante warping.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/F4.9_original_vs_warped.png}
    \caption{Comparación del proceso de warping. (a) Radiografía original con variabilidad de pose y escala. (b) Imagen normalizada geométricamente mediante warping, enfocándose en la región delimitada por los landmarks (margin\_scale=1.05, fill\_rate=47\%).}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Figures/F4.10_margin_scale.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{Figures/F4.11_flujo_normalizacion.png}
