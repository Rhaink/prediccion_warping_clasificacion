...
Con los parámetros seleccionados (\texttt{margin\_scale}=1.05, \texttt{use\_full\_coverage}=falso), el método produce una tasa de llenado (\textit{fill rate}) de aproximadamente 47\%, correspondiente a la región pulmonar sin el fondo circundante. Alternativamente, con la estrategia de cobertura completa (\texttt{use\_full\_coverage}=verdadero), la tasa de llenado alcanza valores superiores al 95\%, dependiendo del preprocesamiento aplicado a las imágenes.
