...
El experimento comparativo de cuatro configuraciones (Original, Normalizado, ALP, Recortado) proporciona evidencia directa de que:

\begin{enumerate}
    \item \textbf{Las imágenes originales aprenden características espurias:} La caída de exactitud al eliminar bordes (3.32 puntos con recorte simple, 2.28 puntos con ALP inteligente) demuestra que las etiquetas hospitalarias contribuyen significativamente. Los resultados reportados por Picazo~\cite{picazo2023tesis} usando el mismo conjunto de datos validan nuestras observaciones.

    \item \textbf{La normalización geométrica aprende características genuinas:} El sistema propuesto (98.60\%) mantiene alto rendimiento usando únicamente la región pulmonar normalizada con 15 puntos de referencia. La superioridad sobre el ALP (96.40\%, 4 puntos de referencia) se atribuye a mayor fidelidad anatómica y normalización geométrica mediante GPA, Warping y SAHS.

    \item \textbf{El recorte inteligente es superior al recorte simple:} El ALP (96.40\%, basado en 4 puntos de referencia) supera al recorte simple (95.36\%, porcentaje fijo) en 1.04 puntos, demostrando que el recorte inteligente que preserva la forma pulmonar es más efectivo. Sin embargo, ambos métodos de recorte son inferiores a la normalización geométrica completa (98.60\%, con 15 puntos de referencia + GPA), que además corrige variabilidad geométrica

    \item \textbf{La exactitud de 98.68\% en imágenes originales está comprometida:} Este valor no refleja capacidad para identificar patologías pulmonares, sino capacidad para explotar correlaciones espurias con artefactos hospitalarios en las esquinas.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Exactitud:} 98.60\% sobre conjunto de prueba de 1,895 imágenes.
    \item \textbf{F1-Macro:} 98.00\%, indicando rendimiento equilibrado entre clases desbalanceadas.
...
La Tabla \ref{tab:cumplimiento_objetivos} presenta el cumplimiento de los objetivos específicos planteados al inicio del trabajo.

\begin{table}[htbp]
    \centering
    \caption{Cumplimiento de objetivos específicos del trabajo de investigación.}
    \label{tab:cumplimiento_objetivos}
    \small
    \setlength{\tabcolsep}{3pt}
    \begin{tabular}{@{}p{1cm}p{6cm}p{5.5cm}c@{}}
        \toprule
        \textbf{Obj.} & \textbf{Descripción} & \textbf{Resultados Obtenidos} & \textbf{Cumplido} \\
        \midrule
        1 & Diseñar, implementar y evaluar un método deformable de alineación y normalización &
        GPA + Triangulación Delaunay + Deformación afín por partes (Cap. 4.4) &
        \checkmark \\
        \midrule
        2 & Proponer un método de extracción y selección de características &
        Ensemble ResNet-18 + Coordinate Attention + TTA: 3.61 px (Cap. 4.3) &
        \checkmark \\
        \midrule
        3 & Evaluar el rendimiento de diferentes clasificadores (KNN y CNN) &
        ResNet-18: 98.60\% accuracy; PCA-Fisher-KNN: 80.44\%. Ambos mejoran con normalización (+3.24 pp y +3.38 pp respectivamente) (Cap. 5.3) &
        \checkmark \\
        \midrule
        4 & Validar el clasificador mediante métricas y validación cruzada &
        Exactitud, F1-Score, matrices de confusión. Validación cruzada k=5: 98.60\% (Cap. 5.3) &
        \checkmark \\
        \midrule
        5 & Contrastar resultados con/sin alineación &
        Original: 98.68\%, Normalizado: 98.60\%, Recortado: 95.36\%. Evidencia de características espurias (Cap. 5.4) &
        \checkmark \\
        \bottomrule
    \end{tabular}
\end{table}

Los cinco objetivos específicos planteados fueron cumplidos satisfactoriamente. En particular, el objetivo 3 se cumplió mediante la evaluación de dos arquitecturas complementarias: ResNet-18 (aprendizaje profundo) y PCA-Fisher-KNN (método tradicional de reducción de dimensionalidad). Ambos clasificadores mostraron mejoras consistentes con la normalización geométrica, validando que el beneficio no es específico de una arquitectura particular sino que refleja una mejora fundamental en la separabilidad entre clases.
...
El sistema basado en normalización geométrica alcanzó 98.60\% de exactitud y 98.00\% de F1-Macro, demostrando que el enfoque propuesto es efectivo para la clasificación de enfermedades pulmonares. Este rendimiento es:
\begin{itemize}
    \item Competitivo con trabajos relacionados en la literatura de clasificación de COVID-19.
    \item Robusto ante variaciones de semilla aleatoria ($\pm$0.26 pp).
...
Se realizó una comparación controlada entre cuatro configuraciones de preprocesamiento:

\begin{itemize}
    \item \textbf{Original + SAHS:} 98.68\% de exactitud.
    \item \textbf{Normalizado + SAHS:} 98.60\% de exactitud.
...
\begin{enumerate}
    ...
    \item \textbf{La normalización geométrica recupera el rendimiento:} El sistema propuesto (98.60\%) supera al recortado (95.36\%) en 3.24 puntos, demostrando que la alineación y enfoque en la región pulmonar permite aprender características patológicas que compensan la ausencia de artefactos.
...
\begin{itemize}
    ...
    \item El sistema propuesto (98.60\%) alcanza alto rendimiento utilizando \textbf{únicamente} la región pulmonar, aprendiendo características patológicas genuinas en lugar de artefactos.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Transferibilidad a otras modalidades:} El enfoque de detección de landmarks + deformación afín por partes puede aplicarse a otras modalidades de imagen médica donde la alineación anatómica es relevante (e.g., resonancia magnética cerebral, mamografía).
...
\begin{enumerate}
    ...
    \item \textbf{Aprendizaje extremo a extremo:} Explorar un enfoque extremo a extremo donde la detección de puntos de referencia y la clasificación se entrenan conjuntamente, permitiendo que el gradiente de clasificación optimice la detección de puntos de referencia.
...
El presente trabajo demostró que la normalización geométrica mediante detección automática de puntos de referencia anatómicos es un enfoque viable y efectivo para la clasificación de enfermedades pulmonares en radiografías de tórax. El sistema propuesto alcanzó 98.60\% de exactitud y 98.00\% de F1-Macro, resultados competitivos con el estado del arte en clasificación de COVID-19.

La contribución principal del trabajo es la \textbf{validación experimental de la hipótesis} y el \textbf{hallazgo de que clasificadores entrenados con imágenes sin procesar aprenden características espurias}. El experimento de recorte demostró que la exactitud de 98.68\% en imágenes originales está inflada por artefactos hospitalarios: al eliminar únicamente las etiquetas de las esquinas, la exactitud cae a 95.36\%. El sistema propuesto (98.60\%) alcanza rendimiento comparable utilizando exclusivamente la región pulmonar, garantizando que las características aprendidas son clínicamente relevantes.
