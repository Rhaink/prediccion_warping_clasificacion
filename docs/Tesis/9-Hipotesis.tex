...
\label{subsec:hipotesis-principal}

Como se establece en la Sección~\ref{sec:introduccion}, la variabilidad geométrica representa un desafío fundamental para los sistemas CAD. Se plantea que la incorporación de restricciones geométricas anatómicas en la función de pérdida de redes neuronales convolucionales \citep{lecun1998gradient}, combinada con normalización espacial mediante warping afín por partes, mejorará significativamente la robustez de clasificadores de patologías pulmonares ante artefactos de compresión y variabilidad geométrica.
...
\begin{itemize}
    \item Función de pérdida: MSE, Wing Loss \citep{feng2018wing}, Wing Loss + restricciones geométricas
    \item Estrategia de entrenamiento: backbone congelado vs. fine-tuning completo
    \item Porcentaje de cobertura (fill rate) del warping: 47\%, 96\%, 99\% (ver definición en Ecuación~\ref{eq:fill-rate})
\end{itemize}
...
La relevancia de estas predicciones se fundamenta en necesidades clínicas concretas, como se detalla en la Sección~\ref{sec:justificacion}.
