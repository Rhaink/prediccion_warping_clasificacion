...
\begin{figure}[htbp]
    ...
    \caption{Representación de los 15 puntos de referencia anatómicos. (a) Radiografía con puntos de referencia etiquetados. (b) Esquema de la distribución espacial, donde el eje central (gris) define la línea media y los puntos laterales delimitan los contornos pulmonares izquierdo (azul) y derecho (naranja).}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        node distance=1.0cm,
        block/.style={rectangle, draw, fill=blue!20, minimum width=1.6cm, minimum height=1.0cm, align=center, font=\small},
        pool/.style={rectangle, draw, fill=green!20, minimum width=1.2cm, minimum height=1.0cm, align=center, font=\small},
        fc/.style={rectangle, draw, fill=orange!20, minimum width=1.6cm, minimum height=1.0cm, align=center, font=\small},
        input/.style={rectangle, draw, fill=gray!20, minimum width=1.6cm, minimum height=1.4cm, align=center, font=\small},
        ...
        % Labels
        \node[below=0.4cm of conv1, font=\footnotesize] {Extracción de características};
        \node[below=0.4cm of fc1, font=\footnotesize] {Clasificación/Regresión};
    \end{tikzpicture}%
    }
    \shorthandon{>}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        node distance=1.2cm,
        block/.style={rectangle, draw, fill=blue!20, minimum width=2.4cm, minimum height=0.9cm, align=center, font=\small},
        sum/.style={circle, draw, fill=yellow!30, minimum size=0.8cm, font=\small},
        arrow/.style={->, >=stealth, thick}
    ]
        % Input
        \node[font=\normalsize] (input) {$\mathbf{x}$};

        % Main path
        \node[block, right=1.4cm of input] (conv1) {Conv 3$\times$3};
        \node[block, right=of conv1] (bn1) {BN + ReLU};
        \node[block, right=of bn1] (conv2) {Conv 3$\times$3};
        \node[block, right=of conv2] (bn2) {BN};

        % Sum
        \node[sum, right=1.2cm of bn2] (sum) {$+$};

        % Output
        \node[right=1.2cm of sum, font=\normalsize] (output) {$\mathbf{y}$};
        ...
        \draw[arrow] (bn2) -- (sum);
        \draw[arrow] (sum) -- node[above, font=\small] {ReLU} (output);

        % Skip connection
        \coordinate (skip_start) at ($(input)+(0.5,0)$);
        \coordinate (skip_top) at ($(skip_start)+(0,1.5)$);
        \coordinate (skip_end) at ($(sum)+(0,1.5)$);
        \draw[arrow, dashed, blue!60!black] (skip_start) -- (skip_top) -- (skip_end) -- (sum);

        % Label
        \node[above=0.15cm of skip_top, font=\small, blue!60!black] {Conexión residual (skip)};

        % F(x) label
        \node[below=0.6cm of bn1, font=\small] {$F(\mathbf{x})$};
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{Figures/F4.7_proceso_gpa.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{Figures/F4.9_original_vs_warped_es.png}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        node distance=1.0cm,
        block/.style={rectangle, draw, fill=blue!20, minimum width=2.0cm, minimum height=0.8cm, align=center, font=\small},
        pool/.style={rectangle, draw, fill=green!20, minimum width=1.8cm, minimum height=0.8cm, align=center, font=\small},
        op/.style={circle, draw, fill=yellow!30, minimum size=0.7cm, font=\small},
        io/.style={rectangle, draw, fill=gray!20, minimum width=1.8cm, minimum height=1.0cm, align=center, font=\small},
        arrow/.style={->, >=stealth, thick}
    ]
        % Fila 1: Pool H path
        \node[pool] (poolh) {Pool H};
        \node[block, right=1.0cm of poolh] (convh) {Conv+$\sigma$};

        % Fila 2 (centro): Input -> Concat -> Multiply -> Output
        \node[io, below=1.5cm of poolh] (input) {Entrada};
        \node[block, right=1.0cm of input] (concat) {Concat+Conv};
        \node[op, right=1.0cm of concat] (mult) {$\times$};
        \node[io, right=1.0cm of mult, fill=orange!20] (output) {Salida};

        % Fila 3: Pool W path
        \node[pool, below=1.5cm of input] (poolw) {Pool W};
        \node[block, right=1.0cm of poolw] (convw) {Conv+$\sigma$};
        ...
        \draw[arrow, dashed, blue!60!black] (input.west) -- ++(-0.5,0) |- ([yshift=-1.2cm]poolw.south) -| (mult.south);

        % Etiqueta (más abajo para no cruzar la línea)
        \node[below=1.4cm of poolw, font=\footnotesize, blue!60!black, xshift=1.2cm] {skip connection};
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        node distance=0.8cm,
        box/.style={rectangle, draw, fill=blue!20, minimum width=2.0cm, minimum height=0.9cm, align=center, font=\small},
        img/.style={rectangle, draw, fill=gray!20, minimum width=1.8cm, minimum height=0.9cm, align=center, font=\small},
        op/.style={rectangle, draw, fill=yellow!30, minimum width=2.0cm, minimum height=0.9cm, align=center, font=\small},
        pred/.style={rectangle, draw, fill=green!20, minimum width=2.2cm, minimum height=0.9cm, align=center, font=\small},
        arrow/.style={->, >=stealth, thick}
    ]
        % === RAMA SUPERIOR ===
        \node[img] (img1) {Original};
        \node[box, right=0.7cm of img1] (ens1) {Ensamble};
        \node[pred, right=0.7cm of ens1] (pred1) {4 predicciones};

        % === RAMA INFERIOR ===
        \node[img, below=1.5cm of img1] (img2) {Reflejada};
        \node[box, right=0.7cm of img2] (ens2) {Ensamble};
        \node[pred, right=0.7cm of ens2] (pred2) {4 predicciones};
        \node[op, right=0.7cm of pred2] (swap) {Intercambio};

        % === PROMEDIO Y SALIDA (a la derecha de swap, centrado entre filas) ===
        \node[op, right=0.7cm of swap, yshift=0.75cm] (avg) {Promedio};
        \node[pred, right=0.7cm of avg, fill=orange!30] (final) {Resultado};

        % === FLECHAS ===
        \draw[arrow] (img1) -- (ens1);
        \draw[arrow] (ens1) -- (pred1);
        \draw[arrow] (pred1.east) -- ++(0.4,0) |- (avg.west);

        \draw[arrow] (img1) -- node[left, font=\footnotesize] {flip} (img2);
