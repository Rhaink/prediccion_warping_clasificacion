# thesis_validation_fisher_massive_cpu.py
# -----------------------------------------------------------------------------
...
def cargar_datos_simples(image_root_dir, split_name):
    ...
    for i, row in df.iterrows():
        ...
        posibles_rutas = [
            Path(image_root_dir) / split_name / categoria / nombre_archivo, # Estructura completa
...
def criterio_fisher_manual(X_pca, y):
    ...
    for i in range(num_componentes):
        ...
        denominador = var_0 + var_1 + 1e-9 # Pequeño valor para evitar división por cero
...
def main():
    # Añadimos argparse para flexibilidad con el dataset masivo
    parser = argparse.ArgumentParser(description="Validación Fisher Básica (CPU)")
    parser.add_argument("--dataset-dir", type=str, default="outputs/warped_dataset", 
                        help="Ruta al dataset (ej: outputs/warped_dataset o outputs/full_warped_dataset)")
    args = parser.parse_args()

    print("="*60)
    print("VALIDACIÓN GEOMÉTRICA - VERSIÓN DIDÁCTICA (CPU/NumPy)")
    print(f"Dataset objetivo: {args.dataset_dir}")
    print("="*60)
    ...
    print("    Evaluando en conjunto de TEST (Datos nunca vistos)...")
    preds = knn.predict(X_test_final)
    
    acc = accuracy_score(y_test, preds)
    print("\n" + "#"*40)
    print(f"RESULTADO FINAL REAL ({Path(dataset_dir).name}): Accuracy = {acc*100:.2f}%")
    print("#"*40)
